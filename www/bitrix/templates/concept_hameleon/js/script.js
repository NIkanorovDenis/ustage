...
function updateBox(button, countBox, idboxEl, action, other_complect){
...
$(document).ready(
    function(){
        ...
        /*first-block arrow-down to next block*/
        if ($(window).width() > 767){
            var timeOut = setTimeout(
                function(){
        ...
        if($(window).width() > 991)
            $('div.first-slider').find(".first-block.hidden-lg").remove();

        if($(window).width() <= 991)
            $('div.first-slider').find(".first-block.hidden-xs").remove();

        $('div.first-slider').slick({
            dots: false,
            infinite: true,
        ...
        if ($(".wrap-first-slider").hasClass('min-height-block')){
            if ($(window).height() > $(".wrap-first-slider").height())
                $(".wrap-first-slider").find(".slick-track").css("height", $(window).height() + "px");

        }

        /*slider-advantages*/
        $('div.slider-advantages.big-slide').slick({
            dots: true,
            adaptiveHeight: false
        });

        $('div.slider-advantages.small-slide').slick({
            dots: true,
            slidesToShow: 2,
            adaptiveHeight: false,
            slidesToScroll: 2,
        ...
        $('.slider-services').each(
            function(i, elem){
                var sliderHeightMax = 0;

                $(this).find(".element-table").each(
                    function(){
                ...
                $(this).find(".element-table").height(sliderHeightMax);
                ...
                $(this).slick({
                    ...
                    swipe: false,
                    touchMove: false,
                    asNavFor: $(this).parents("div.block").find('.slider-services-images')
                });

                $(this).parents("div.block").find('.slider-services-images').slick({
                    slidesToShow: 1,
                    slidesToScroll: 1,
                    asNavFor: $(this).parents("div.block").find('.slider-services'),
                    centerPadding: 0,
                    arrows: false,
                    speed: 0,
                    fade: true,
            ...
            function(i, elem){
                ...
                if (count == 1){
                    $('.slider-for', $(this)).slick({
                        ...
                        appendArrows: $(this).find('.slider-nav-wrap')
                    });
        ...
        $(".slider-gallery").each(
            function(){
                ...
                var count = parseInt($(this).attr("data-slide-visible"));
    ...
    function(){
    	...
    	if($(".menu-type2").hasClass("active") || $(".menu-type3").hasClass("active")){
            ...
            if ($(document).width() > 767){
                ...
                var size = menu.find('li.lvl1').size();
        ...
        if ($(window).width() >= 1200){
            $('iframe.video-bg').each(
                function(){

                    var height = $(this).parents('div.video-bg').outerHeight();
                    var videoThis = $(this);
        ...
        if ($(window).width() > 767)
        {
            ...
            $(".tarif[data-round-height = 'Y']").each(
                function(){
                    ...
                    if($(window).width() >= 1200)
                        cols = $(this).attr("data-col-lg");

                    if($(window).width() >= 992 && $(window).width() <= 1199)
                        cols = $(this).attr("data-col-md");

                    if($(window).width() >= 768 && $(window).width() <= 991)
                        cols = $(this).attr("data-col-sm");

                    var max_height = 0;

                    $(this).find(".tarif-element-inner").each(
                        function(index){
...
$(document).on("click", "a.scroll",
    function(){
        ...
        if ($(window).width() < 767){
            var destination = $(elementClick).offset().top - heightOut;
...
$(document).on("click", "div.mob-tab",
    function(){
        ...
        if (!$(this).hasClass("active")){
            ...
            $("div.mob-content", block).slideDown(200,
                function(){
...
$(document).on("click", ".count-plus",
    function(){
        ...
        value += step;
        value = parseCount(value, step, minVal);
