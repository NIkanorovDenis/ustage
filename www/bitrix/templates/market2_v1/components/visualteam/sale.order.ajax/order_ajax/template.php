 ...
 * @var CUser $USER
 * @var SaleOrderAjaxExtended $component
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<input type="hidden" size="30" name="ORDER_PROP_48" id="soa-property-48" placeholder="" value="Корзина">
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<!--	REGION BLOCK	-->
				<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-noactive">
					<div class="bx-soa-section-title-container">
						<h2 class="bx-soa-section-title col-sm-9">
				...
				<? else: ?>
					...
					<!--	PAY SYSTEMS BLOCK	-->
					<div id="bx-soa-paysystem" data-visited="true" class="bx-soa-section bx-noactive" <?=($hidePaySystem ? 'style="display:none"' : 'style="display:none"')?>>
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-count"></span><?=$arParams['MESS_PAYMENT_BLOCK_NAME']?>
							</h2>
        ...
        <?php $isBonusFieldVisible = $arResult['PERSON_TYPE'][1]['CHECKED'] === 'Y'; ?>
        <!--	BONUS BLOCK	-->
        <div id="bx-soa-bonus" data-visited="false" class="bx-soa-section bx-active" style="display: <?= $isBonusFieldVisible ? 'block' : 'none' ?>;">
          <div class="bx-soa-section-title-container">
            <h2 class="bx-soa-section-title col-sm-9">
              <span class="bx-soa-section-title-count"></span>Бонусная карта USTAGE BONUS
            </h2>
            <!-- <div class="col-xs-12 col-sm-3 text-right"><a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a></div> -->
          </div>
          <div class="bx-soa-section-content container-fluid">
            <div class="form-group bx-soa-customer-field" data-property-id-row="42">
              <label for="soa-property-42" class="bx-soa-custom-label">Номер карты</label>

              <div class="soa-property-container">
                <input type="text" size="100" name="ORDER_PROP_42" id="soa-property-42" placeholder="" class="form-control bx-soa-customer-input bx-ios-fix">
              </div>

              <div class="soa-hint">Поле необязательно для заполнения, Вы можете озвучить номер карты менеджеру</div>
            </div>

            <div class="soa-text">
              <p>Если у Вас нет карты, получить ее можно <a href="https://backend.pluscards.ru/program/121250/invite/344#/login" target="_blank">тут</a>.</p>
              <p>Окончательную стоимость заказа с учетом бонусной карты, Вам озвучит менеджер.</p>
              <p><a href="/ustage-bonus/" target="_blank">Условия бонусной программы</a></p>
            </div>
          </div>
        </div>
          ...
          <div id="bx-soa-bonus-hidden" class="bx-soa-bonus"></div>
				</div>
			</div>
