...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
      ...
      this.bonusBlockNode = BX(parameters.bonusBlockId);
      this.bonusHiddenBlockNode = BX(parameters.bonusBlockId + '-hidden');
		...
		refreshOrder: function(result)
		{
			...
			if (result.order.PERSON_TYPE[0].CHECKED === 'Y') {
				$('#bx-soa-bonus').show();
			} else {
				$('#bx-soa-bonus').hide();
			}
			...
			for (k in errors)
			{
				...
				switch (k.toUpperCase())
				{
					...
					case 'PROPERTY':
						...
						break;
          case 'BONUS':
            if (showAll || this.bonusBlockNode.getAttribute('data-visited') === 'true')
            {
              this.showError(this.bonusBlockNode, blockErrors, true);
              this.showError(this.bonusHiddenBlockNode, blockErrors);
            }
            break;
				}
			}
		...
		clickOrderSaveAction: function(event)
		{
			if (this.isValidForm())
			{
				...
				if (!document.getElementById('accept_form_one_click_order').checked){
						document.getElementById('accept_form_one_click_order').parentNode.classList.add('error');
				}
				else if (!document.getElementById('accept_form_one_click_order2').checked){
						document.getElementById('accept_form_one_click_order2').parentNode.classList.add('error');
				}
				else
				{
					this.doSaveAction();
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				//PROPERTY_PICTURES_SKU_VALUE
				if (currentColumn.id === 'PROPERTY_PICTURES_SKU_VALUE') {
					var sku_content = this.createBasketItemColumn(currentColumn, item, active);
					if (sku_content != '') {
						mainColumns.push(sku_content);
						sku_find = true;
					}
					continue;
				}
		...
		createBasketItemImg: function(data)
		{
			...
			if (logotype && logotype.src_2x)
			{
				logoNode.setAttribute('style',
					'background-image: url("' + logotype.src_2x + '");'
				);
			}
			else
			{
		...
		getDeliveryPriceNodes: function(delivery)
		{
			...
			{
				priceNodesArray = [ delivery.PRICE>0 ? 'от ' + delivery.PRICE_FORMATED : delivery.PRICE_FORMATED];
			}
				...
				labelNodes.push(
					BX.create('DIV', {
						...
						html: typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined'
							? (item.DELIVERY_DISCOUNT_PRICE>0 ? 'от ' + item.DELIVERY_DISCOUNT_PRICE_FORMATED : item.DELIVERY_DISCOUNT_PRICE_FORMATED)
							: (item.PRICE>0 ? 'от ' + item.PRICE_FORMATED : item.PRICE_FORMATED)})
				);
			}
				...
				labelNodes.push(
					BX.create('DIV', {
						...
						html: typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'
							? (deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED>0 ? 'от ' + deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED : deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED) 
							: (deliveryCached.PRICE>0 ? 'от ' + deliveryCached.PRICE_FORMATED : deliveryCached.PRICE_FORMATED)})
				);
			}
