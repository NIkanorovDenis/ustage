...
<?foreach($arResult["ELEMENTS"] as $i => $arItem) { 
	/**/
	$dbElementGroups = CIBlockElement::GetElementGroups($arItem["ID"], true);
	...
	<?foreach($arResult["CATEGORIES"] as $category_id => $arCategory):?>
		<?foreach($arCategory["ITEMS"] as $i => $arItem):?>
			...
			?>
				...
				<a class="bx_item_block_href qwe" href="<?echo $arItem["URL"]?>">
					...
					<?endif;?>
                    <?
                    $arEl = CIBlockElement::GetByID($arItem["ITEM_ID"])->Fetch();
                    $arSe = CIBlockSection::GetByID($arEl["IBLOCK_SECTION_ID"])->Fetch();
                    ?>
                    <?if($arSe["NAME"] != ""):?>
                        <span style="font-size: 10px;"><?=$arSe["NAME"]?></span>
                    <?endif;?>
				</a>
			<?endif;?>
		<?endforeach;?>
