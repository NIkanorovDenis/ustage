...
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		<?if($arParams["~SHOW_TITLES"] != 'N'):?>
			<div class="h4 bx-auth-title"><?=GetMessage("socserv_as_user")?></div>
		...
		<div class="bx-auth-service-form" id="bx_auth_serv<?=$arParams["SUFFIX"]?>" style="display:none">
			<?foreach($arAuthServices as $service):?>
				<?if(($arParams["~FOR_SPLIT"] != 'Y') || (!is_array($service["FORM_HTML"]))):?>
					<div id="bx_auth_serv_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>" style="display:none"><?=str_replace(array('input type="text"', 'name="" value="Войти"'), array('input type="text" class="form-control" ', 'name="enter" value="Войти"'), $service["FORM_HTML"]) ?></div>
				<?endif;?>
			<?endforeach?>
		</div>
