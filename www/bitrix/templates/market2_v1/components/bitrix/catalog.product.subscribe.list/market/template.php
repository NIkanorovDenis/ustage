...
if(!$arResult['USER_ID'] && !isset($arParams['GUEST_ACCESS'])):?>
	...
	<div class="row">
		...
		<div class="col-md-8 col-sm-7 catalog-subscriber-identification-form">
			<div class="h4"><?=Loc::getMessage('CPSL_HEADLINE_FORM_SEND_CODE')?></div>
			<hr class="bxe-light">
			<form method="post">
				<?=bitrix_sessid_post()?>
		...
		<div class="col-md-8 col-sm-7">
			<div class="h4"><?=Loc::getMessage('CPSL_HEADLINE_FORM_FOR_ACCESSING')?></div>
			<hr class="bxe-light">
			<form method="post">
				<?=bitrix_sessid_post()?>
				<div class="form-group">
...
if (!empty($arResult['ITEMS']))
{
	...
	<?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) // Simple Product
	{
		...
		<div class="bx_catalog_item_controls">
			<div>
				<a class="bxr-color-button" href="<?=$arItem['DETAIL_PAGE_URL'];?>">
					<?=GetMessage('CPSL_TPL_MESS_BTN_DETAIL');?>
				</a>
			</div>
			<div>
				<a id="<?=$arItemIDs['SUBSCRIBE_DELETE_LINK'];?>" class="delete-subscribe" href="javascript:void(0)">
...
else
{
	if(isset($arParams['GUEST_ACCESS'])):
		echo '<div class="h3">'.Loc::getMessage('CPSL_SUBSCRIBE_NOT_FOUND').'</div>';
	endif;
}
?>