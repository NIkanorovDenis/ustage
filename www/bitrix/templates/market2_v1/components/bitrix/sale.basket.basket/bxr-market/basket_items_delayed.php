		...
		<tbody>
			...
			?>
				<tr id="<?=$arItem["ID"]?>">
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
													...
													<div class="bx_size_scroller_container">
														<div class="bx_size">
															<ul id="prop_<?=$arProp["CODE"]?>_<?=$arItem["ID"]?>" style="width: 200%; margin-left: <?=$marginLeft; ?>;">
																...
																?>
																	<li style="width:10%;"<?=$selected?>>
																		<a href="javascript:void(0);" class="cnt bxr-border-color-hover<?=$selectedChild?>"><?=htmlspecialcharsbx($arSkuValue["NAME"]); ?></a>
																	</li>
																<?
																endforeach;
																unset($counter);
						...
						?>
							<td class="price bxr-price-line bxr-center-mobile">
								...
								<?else:?>
									<div class="current_price"><?= Alexkova\Market2\Core::bxrFormatPrice($arItem["PRICE_FORMATED"], false, true)?></div>
								<?endif?>
						...
						?>
							<td class="custom bxr-price-line bxr-center-mobile">
                                <div class="bxr-sum-price" id="sum_<?=$arItem["ID"]?>">
                                    <?=Alexkova\Market2\Core::bxrFormatPrice($arItem[$arHeader["id"]], false, true);?>
                                </div>
							</td>
						<?
						endif;
					...
					?>
						<td class="control">
                            <a class="bxr-icon-button-basket bxr-icon-button-add-big-basket bxr-border-color-hover" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?>" title="<?=GetMessage("SALE_ADD_TO_BASKET")?>"><span class="fa fa-shopping-basket" aria-hidden="true"></span></a>
							<?
							if ($bDeleteColumn):
							?>
