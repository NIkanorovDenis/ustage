		...
		<table id="basket_items">
			...
			<tbody>
				...
				foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
					...
					$SKU_IMG = '';
					foreach ($arItem['PROPS'] AS $props_item) {
						if ($props_item['CODE'] == 'PICTURES_SKU') {
							if (!empty($props_item['~VALUE'][0])) $SKU_IMG = CFile::GetPath($props_item['~VALUE'][0]);
						}
					}
					...
					>
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								<td class="itemphoto">
									<div class="bx_ordercart_photo_container">
										...
										else if (strlen($arItem["PREVIEW_PICTURE_SRC"]) > 0):
								...
								<td class="item">
									<div class="h2 bx_ordercart_itemtitle">
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"] ?>"><?endif;?>
											<?=$arItem["NAME"]?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									</div>
														...
														<div class="bx_scu">
															<ul id="prop_<?=$arProp["CODE"]?>_<?=$arItem["ID"]?>"
																...
																foreach ($arProp["VALUES"] as $valueId => $arSkuValue):
                                                                    ...
                                                                    $selectedChild = ($selectedIndex == $counter ? ' bxr-border-color' : '');
																?>
																	<li style="width:10%;"
																	...
																	>
																		<a href="javascript:void(0)" class="cnt bxr-border-color-hover<?=$selectedChild?>"><span class="cnt_item" style="background-image:url(<?=$arSkuValue["PICT"]["SRC"];?>)"></span></a>
																	</li>
																<?
																endforeach;
																unset($counter);
																...
																if (!empty($arProp["VALUES"]))
																{
																	...
																	?>
																		<li style="width:10%;"
																			...
																			data-property="<?=$arProp["CODE"]?>"
																		>
																			<a href="javascript:void(0)" class="cnt bxr-border-color-hover<?=$selectedChild?>"><?=$skuValueName; ?></a>
																		</li>
																	<?
																		unset($skuValueName);
																	endforeach;
							...
							?>
								<td class="custom bxr-price-line bxr-center-mobile">
                                    <div class="bx-basket-qty-line">
                                        <table cellspacing="0" cellpadding="0" class="counter">
                                            <tr>
                                                <td>
                                                    <div class="bxr-basket-qty">
                                                        <?
                                                    ...
                                                    <?if (isset($arItem["MEASURE_TEXT"])):?>
                                                        <div class="bxr-measure"><?=$arItem["MEASURE_TEXT"]?></div>
                                                    <?endif;?>
                                                </td>
                                            </tr>
                                        </table>
							...
							elseif ($arHeader["id"] == "PRICE"):
							?>
								<td class="price bxr-price-line bxr-center-mobile">
										...
										<div class="current_price" id="current_price_<?=$arItem["ID"]?>">
                                            <?= Alexkova\Market2\Core::bxrFormatPrice($arItem["PRICE_FORMATED"], false, true)?>
										</div>
							...
							?>
								<td class="custom bxr-center-mobile">
                                    <div>
                                        <span><?=$arHeader["name"]; ?>:</span>
                                        <div id="discount_value_<?=$arItem["ID"]?>" class="bxr-discount-value"><?=$arItem["DISCOUNT_PRICE_PERCENT_FORMATED"]?></div>
                                    </div>
								</td>
							<?
							elseif ($arHeader["id"] == "WEIGHT"):
							...
							?>
								<td class="custom bxr-price-line bxr-center-mobile">
									...
									?>
                                        <span><?=$arHeader["name"]; ?>:</span>
										<div class="bxr-sum-price" id="sum_<?=$arItem["ID"]?>">
									<?
									endif;

                                    echo Alexkova\Market2\Core::bxrFormatPrice($arItem[$arHeader["id"]], false, true);
						...
						?>
							<td class="control">
                                ...
                                if ($bDelayColumn):
                                    ?>
                                    <a class="bxr-icon-button-basket bxr-icon-button-delay-big-basket bxr-border-color-hover" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>" title="<?=GetMessage("SALE_DELAY")?>"><span class="fa fa-cube" aria-hidden="true"></span></a>
                                    <?
                                endif;
                                if ($bDeleteColumn):
                                    ?>
                                    <a class="bxr-icon-button-basket bxr-icon-button-delete-big-basket bxr-border-color-hover" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" title="<?=GetMessage("SALE_DELETE")?>"><span class="fa fa-close" aria-hidden="true"></span></a>
                                    <?
                                endif;
                                ?>
							</td>
	...
	<div class="bx_ordercart_order_pay">
		...
		?>
			<div class="bx_ordercart_coupon">
				<span><?=GetMessage("STB_COUPON_PROMT")?></span><input type="text" id="coupon" name="COUPON" value="" onchange="enterCoupon();"><a class="bxr-color-button" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
			</div><?
...
?>
			...
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					<tr>
						...
						<td class="bxr-basket-all-sum" id="allSum_FORMATED"><?= Alexkova\Market2\Core::bxrFormatPrice($arResult["allSum_FORMATED"], false, true)?></td>
					</tr>
		...
		<div class="bx_ordercart_order_pay_center">
            ...
            <?if(!isset($arParams["PRINT_ORDER"]) || $arParams["PRINT_ORDER"] == "Y" ):?>
                <a href="?print=y" class="print_order" style="background: none !important;" target="_blank"  ><i class="fa fa-print" aria-hidden="true"></i><?=GetMessage("PRINT")?></a>
            <?endif;?>
            <?
            ...
            if ($bxr_one_click_order_basket == "Y") {
                ?><span class="bxr-border-color bxr-font-color bxr-one-click-order-basket" data-toggle="modal" data-target="#bxr-one-click-order"><?=GetMessage("ONE_CLICK_ORDER")?></span><?
            }?>
			<?
			if ($arParams["AUTO_CALCULATION"] != "Y")
			{
				?>
				<a href="javascript:void(0)" onclick="updateBasket();" class="checkout refresh bxr-color-button"><?=GetMessage("SALE_REFRESH")?></a>
				<?
			}
			?>
			<a href="javascript:void(0)" onclick="checkOut();" class="checkout bxr-color-button"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?
