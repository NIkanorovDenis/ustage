    ...
    if(isset($arResult["ITEMS"]) && is_array($arResult["ITEMS"])) {
        foreach ($arResult["ITEMS"] as $k => $v) {
            ...
            if(isset($v["IPROPERTY_VALUES"]) && is_array($v["IPROPERTY_VALUES"])) {
                $arResult["ITEMS"][$k]["IPROPERTY_VALUES"] = \Alexkova\Market2\Bxmarket::clearRegionMegaTags($arResult["ITEMS"][$k]["IPROPERTY_VALUES"]);
            }
        }
    }
}
...
if (strlen(COption::GetOptionString('alexkova.market2', 'list_marker_type'))>0){
        $bxreadyMarkers = COption::GetOptionString('alexkova.market2', 'list_marker_type');
}else{
