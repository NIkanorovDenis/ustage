...
<div class="subscription bxr-subscription">
	<form action="<?=$arResult["FORM_ACTION"]?>?type=list" method="post">
		...
		<table cellspacing="0" class="subscription-layout">
			<tr>
				<td class="field-name"><?echo GetMessage("CT_BSE_EMAIL_LABEL")?></td>
                                <td class="field-form">
                                    <input type="text" name="EMAIL" value="<?echo $arResult["SUBSCRIPTION"]["EMAIL"]!=""? $arResult["SUBSCRIPTION"]["EMAIL"]: $arResult["REQUEST"]["EMAIL"];?>" class="subscription-email bxr-border-color-focus" />
				</td>
			</tr>
                <tr>
                    <td class="field-name subscription-format-td"><?echo GetMessage("CT_BSE_FORMAT_LABEL")?></td>
                    <td class="field-form">
			...
			<tr>
				<td class="field-name field-rubric"><?echo GetMessage("CT_BSE_RUBRIC_LABEL")?></td>
				<td class="field-form">
					<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
						<div class="subscription-rubric">
							<input type="checkbox" id="RUBRIC_<?echo $itemID?>" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> /><label for="RUBRIC_<?echo $itemID?>"><b><?echo $itemValue["NAME"]?></b></label>
	...
	<?if($arResult["ID"]>0 && $arResult["SUBSCRIPTION"]["CONFIRMED"] <> "Y"):?>
        <div class="subscription-utility">
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-8">
                            <input name="CONFIRM_CODE" type="text" class="subscription-textbox bxr-border-color-focus" value="<?echo GetMessage("CT_BSE_CONFIRMATION")?>" onblur="if (this.value=='')this.value='<?echo GetMessage("CT_BSE_CONFIRMATION")?>'" onclick="if (this.value=='<?echo GetMessage("CT_BSE_CONFIRMATION")?>')this.value=''" />
                        </div>
                        <div class="col-md-4">
                            <input class="bxr-color-button bxr-submit" type="submit" name="confirm" value="<?echo GetMessage("CT_BSE_BTN_CONF")?>" />
                        </div>
                    </div>
                </div>
        </div>
	...
	<div class="subscription-utility">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-8">
                        <input name="sf_EMAIL" type="text" class="subscription-textbox bxr-border-color-focus" value="<?echo GetMessage("CT_BSE_EMAIL")?>" onblur="if (this.value=='')this.value='<?echo GetMessage("CT_BSE_EMAIL")?>'" onclick="if (this.value=='<?echo GetMessage("CT_BSE_EMAIL")?>')this.value=''" />
                    </div>
                    <div class="col-md-4">
