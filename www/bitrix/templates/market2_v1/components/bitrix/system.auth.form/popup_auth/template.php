...
<div class="login_page action-form-t bxr-form-body-container">
	<?if (!empty($arResult["ERROR_MESSAGE"])):?>
            <div class="bxr-form-errors"><?
                $arResult["ERROR_MESSAGE"]["MESSAGE"] = explode("<br>", $arResult["ERROR_MESSAGE"]["MESSAGE"]);
                foreach ($arResult["ERROR_MESSAGE"]["MESSAGE"] as $k => $v) {
                    if(!empty($v))
	...
	<form name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=SITE_DIR?>auth/<?//=$arResult["AUTH_URL"]?>" class="bx_auth_form">
		...
		<input placeholder="<?=GetMessage('AUTH_LOGIN_FIELD');?>" class="input_text_style form-control" type="text" name="USER_LOGIN" maxlength="255" value="<?=$arResult["LAST_LOGIN"]?>" />
