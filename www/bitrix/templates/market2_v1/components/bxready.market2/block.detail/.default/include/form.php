...
<?if (is_array($arParams['BXR_FORMS']) && count($arParams['BXR_FORMS'])>0):
	...
	<div class="bxr-contact-form">
        ...
        <?endif;?>
		<button class="<?=$arParams['BXR_FORMS']['EVENT_CLASS']?>" href="javascript:void(0);" data-toggle="modal" data-pid="<?=$arResult["ID"]?>" data-target="#<?=$arParams['BXR_FORMS']['BXR_FORM_ID']?>"
         data-show-img="<?=$arParams["BXR_SHOW_IMG_IN_FORM"]?>" data-show-name="<?=$arParams["BXR_SHOW_NAME_IN_FORM"]?>">
