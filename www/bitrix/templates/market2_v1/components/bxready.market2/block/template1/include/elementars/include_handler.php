...
if (isset($includeAreaName) && strlen($includeAreaName)>0 && strlen($includeType)>0){
	...
	if (strlen($elementarArea) > 0){
		include($elementarArea);
	}else if(file_exists(dirname(__FILE__)."/".$includeType.'/'.$includeAreaName.'.php')) {
		include($includeType.'/'.$includeAreaName.'.php');
	}
