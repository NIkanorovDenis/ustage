        ...
        if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
        {
            ...
            if ($bParam)
            {
                ...
                else
                {
                    if ($value != '')
                    {
                    	if($iblockProperty['NAME'] == "Размер" && !preg_match("/[0-9]+/", $value))
						{
							$strProperty .= '<param name="'.yandex_text2xml($iblockProperty['NAME'], true).'" unit="INT">'.
...
if (!function_exists('yandexPrepareItems'))
{
    function yandexPrepareItems(array &$list, array $parents, array $options)
    {
        foreach (array_keys($list) as $index)
        {
            ...
            {
                $db_props = CIBlockElement::GetProperty($row["IBLOCK_ID"], $row["ID"], array("sort"=>"asc"), array("CODE"=>"MORE_PHOTO"));
                if($ar_props = $db_props->Fetch())
                {
                    if(intval($ar_props["VALUE"]) > 0)
                    {
                        $pictureFile = CFile::GetFileArray($ar_props["VALUE"]);
                        if (!empty($pictureFile))
                        {
                            if (strncmp($pictureFile['SRC'], '/', 1) == 0)
                                $picturePath = $options['PROTOCOL'].$options['SITE_NAME'].CHTTP::urnEncode($pictureFile['SRC'], 'utf-8');
            ...
            {
                $row['PREVIEW_TEXT'] = strip_tags($row['PREVIEW_TEXT']);
            ...
            if($row['DESCRIPTION'] == "")
            {
                $row['DETAIL_TEXT'] = strip_tags($row['DETAIL_TEXT']);
                $row['DESCRIPTION'] = yandex_text2xml(
                    TruncateText(
                        $row['DETAIL_TEXT_TYPE'] == 'html'
                            ? strip_tags(preg_replace_callback("'&[^;]*;'", 'yandex_replace_special', $row['DETAIL_TEXT']))
                            : preg_replace_callback("'&[^;]*;'", 'yandex_replace_special', $row['DETAIL_TEXT']),
                        $options['MAX_DESCRIPTION_LENGTH']
                    ),
                    true
                );
...
if (empty($arRunErrors))
{
    ...
    $esStore = array();
    $rsData = $DB->Query("SELECT * FROM b_catalog_store_product WHERE STORE_ID IN (5,6)");
    while($esItem = $rsData->Fetch())
    {
        $esStore[$esItem["STORE_ID"]][$esItem["PRODUCT_ID"]] = $esItem["AMOUNT"];
    }
        ...
        if (!empty($items))
        {
            foreach ($itemIdsList as $id)
            {
                ...
                if ($row['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
                {
                    ...
                    $db_props = CIBlockElement::GetProperty($row["IBLOCK_ID"], $row["ID"], array("sort"=>"asc"), array("CODE"=>"YMC"));
                    if($ar_props = $db_props->Fetch()) $row['CATEGORY_ID'] = $ar_props["VALUE"];
                    ...
                    foreach ($row['OFFERS'] as $offer)
                    {
                        ...
                        $available = ' available="'.(intval($offer['CATALOG_QUANTITY']) > 0 ? 'true' : 'false').'"';
                        if(intval($esStore[6][$offer['ID']]) > 0) $available = ' available="true"';
                        $itemsContent .= '<offer id="'.$offer['ID'].'"'.$productFormat.$available.">\n";
						if($available != ' available="true"')
						{
							$itemsContent .= '<pickup-options>'."\n";
							$itemsContent .= '<option cost="0" days="2-3"/>'."\n";
							$itemsContent .= '</pickup-options>'."\n";
						}
                        unset($available);
                        ...
                        if (!empty($picture)) $itemsContent .= "<picture>".$picture."</picture>\n";
                        unset($picture);

                        if(intval($esStore[6][$offer['ID']]) > 0) $itemsContent .= "<store>true</store>\n";
                        else $itemsContent .= "<store>false</store>\n";
                ...
                elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
                {
                    ...
                    $available = ' available="'.(intval($row['CATALOG_QUANTITY']) > 0 ? 'true' : 'false').'"';
                    if(intval($esStore[6][$row['ID']]) > 0) $available = ' available="true"';
                    ...
                    if (!empty($row['PICTURE'])) $itemsContent .= "<picture>".$row['PICTURE']."</picture>\n";

                    if(intval($esStore[6][$row['ID']]) > 0) $itemsContent .= "<store>true</store>\n";
                    else $itemsContent .= "<store>false</store>\n";
