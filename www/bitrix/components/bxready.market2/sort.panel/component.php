...
if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
    \Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:sort.panel");
...
while ($property = $propertyIterator->fetch())
{
		...
		if($propertyCode == $arParams["CATALOG_DEFAULT_SORT"])
			$arParams["CATALOG_DEFAULT_SORT"] = "PROPERTY_".$propertyCode;
        
        if ($propertyCode == 'MINIMUM_PRICE')
            $property['NAME'] = GetMessage('PRICE_NAME');
        
        $arProperty["PROPERTYSORT_".$property['CODE']] = $property['NAME'];
        if(in_array(strtoupper($propertyCode), $arParams["ELEMENT_SORT_FIELD"]) || in_array(strtolower($propertyCode), $arParams["ELEMENT_SORT_FIELD"])
        || in_array(strtoupper("PROPERTY_".$propertyCode), $arParams["ELEMENT_SORT_FIELD"]) || in_array(strtolower("PROPERTY_".$propertyCode), $arParams["ELEMENT_SORT_FIELD"]))
...
if(count($arStripCodeSort)>0) {
    foreach ($arStripCodeSort as $k => $v) {
        if(in_array(strtoupper($v), $arParams["ELEMENT_SORT_FIELD"]) || in_array(strtolower($v), $arParams["ELEMENT_SORT_FIELD"]))
...
if (isset($_SESSION["USER_SORTPANEL"]) && is_array($_SESSION["USER_SORTPANEL"]) && !empty($_SESSION["USER_SORTPANEL"]) && count($_SESSION["USER_SORTPANEL"])>0)
{
