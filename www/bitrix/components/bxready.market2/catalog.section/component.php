...
if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
    \Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:catalog.section");
/*************************************************************************
...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	$mFilter = array_merge($arrFilter, $arFilter);
	if ($arParams['SEO_SECTION'] == 'Y') {
		unset($mFilter['SECTION_ID']);
	}

	//EXECUTE
	$rsElements = CIBlockElement::GetList($arSort, $mFilter, false, $arNavParams, $arSelect);
	...
	if ($arParams['USE_FILTER_SORT'] == "Y") {
        foreach ($arrFilter['ID'] as $elementId) {
