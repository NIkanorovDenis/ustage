...
class CatalogSectionComponent extends ElementList
{
	...
	{	
		if (isset($params['ELEMENT_SORT_FIELD']) && is_array($params['ELEMENT_SORT_FIELD'])) {
			$params['ELEMENT_SORT_FIELD'] = $params['ELEMENT_SORT_FIELD'][0];
		}
		$params = parent::onPrepareComponentParams($params);
                
                if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
                    \Alexkova\Bxready2\Component::prepareParams($params, "bxready.market2:catalog.section");

                if(!empty($params["REGION"])) {
                    if( empty($this->globalFilter) ) {
                        $this->globalFilter = array(
                            array(
                                "PROPERTY_BXR_REGION" => array($params["REGION"], false),
                            ),
                        );
                    }
                    else {
                        $this->globalFilter = array_merge($this->globalFilter, array(array("PROPERTY_BXR_REGION" => array($params["REGION"], false))));                    
                    }
                }
		...
		if ($params['DISABLE_INIT_JS_IN_COMPONENT'] !== 'Y')
		{
			CJSCore::Init(array('popup'));
                ...
                //clear params
                if (isset($params["COMPONENT_TEMPLATE"]) && $params["COMPONENT_TEMPLATE"] == "bxready")  {
                        $arUnset = array(
                                'PRODUCT_ROW_VARIANTS',
                                'PRODUCT_BLOCKS_ORDER'
                        );
                        foreach($arUnset as $val){
                		unset($params[$val]);
                        }
                }
		...
		if ($this->initSectionResult())
		{
			...
			parent::processResultData();
                        ...
                        if (Loader::includeModule('alexkova.market2')) {
                            \Alexkova\Market2\Bxmarket::storesElemenQuantity($this->arResult["ITEMS"], $this->arParams);
                        }
                }
	}
	...
	protected function initSectionResult()
	{
		...
		// use SEO sections
		if ($this->arParams['SEO_SECTION'] == 'Y') {
			unset($filterFields['ACTIVE']);
			unset($filterFields['GLOBAL_ACTIVE']);
            $filterFields['INCLUDE_SUBSECTIONS'] = 'Y';
		}
		...
		else
		{
			...
			if ($this->arResult['ID'] > 0 && $this->arParams['ADD_SECTIONS_CHAIN'])
			{
				...
				while ($path = $pathIterator->GetNext())
				{
					...
					$path['IPROPERTY_VALUES'] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($ipropValues->getValues());
	...
	protected function initSectionProperties()
	{
		...
		if ($arResult['ID'] > 0)
		{
			...
			$arResult['IPROPERTY_VALUES'] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($ipropValues->getValues());
		}
	...
	protected function getFilter()
	{
		...
		if ($this->arParams['SEO_SECTION'] == 'Y') {
            ...
            $filterFields['INCLUDE_SUBSECTIONS'] = 'Y';
			unset($filterFields['SECTION_GLOBAL_ACTIVE']);
			unset($filterFields['SECTION_ID']);
			unset($filterFields['ACTIVE']);
			unset($filterFields['~SECTION_ID']);
//			unset($filterFields['CATALOG_AVAILABLE']);
		}
	...
	protected function initMetaData()
	{
		...
		if ($this->arParams['ADD_SECTIONS_CHAIN'] && is_array($this->arResult['PATH']))
		{
			foreach ($this->arResult['PATH'] as $path)
			{
				...
				{
					$APPLICATION->AddChainItem(\Alexkova\Market2\Bxmarket::replaceRegionMegaTags($path['IPROPERTY_VALUES']['SECTION_PAGE_TITLE']), $path['~SECTION_PAGE_URL']);
				}
				else
				{
					$APPLICATION->AddChainItem(\Alexkova\Market2\Bxmarket::replaceRegionMegaTags($path['NAME']), $path['~SECTION_PAGE_URL']);
				}
			}
		}
	...
	protected function processElement(array &$element)
	{
		...
		parent::processElement($element);
                ...
                if(isset($element["IPROPERTY_VALUES"]) && is_array($element["IPROPERTY_VALUES"])) {
                    $element["NAME"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["NAME"]);
                    $element["~NAME"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["~NAME"]);
                    $element["PREVIEW_TEXT"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["PREVIEW_TEXT"]);
                    $element["~PREVIEW_TEXT"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["~PREVIEW_TEXT"]);
                    $element["DETAIL_TEXT"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["DETAIL_TEXT"]);
                    $element["~DETAIL_TEXT"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["~DETAIL_TEXT"]);
                    $element["IPROPERTY_VALUES"] = \Alexkova\Market2\Bxmarket::replaceRegionMegaTags($element["IPROPERTY_VALUES"]);
                }
	...
	protected function initElementList()
	{
		...
		if (!empty($this->elements) && is_array($this->elements))
		{
			...
			if ($this->arParams['USE_FILTER_SORT'] == 'Y')
                            $this->sortByFilterOrder($this->elements);
		}
	}

	protected function sortByFilterOrder(&$elements)
    {
        foreach ($this->globalFilter['ID'] as $elementId) {
