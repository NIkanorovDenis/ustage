...
?><div class="catalog-section bx-<?=$arParams['TEMPLATE_THEME']?>" data-entity="container-<?=$navParams['NavNum']?>"><?
    if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS']))
    {
        ...
        foreach ($arResult['ITEM_ROWS'] as $rowData)
        {
                ...
                $thisUncumId++;
                $generalParams['UNICUM_ID'] = $thisUncumId."_bx";
                ?>
                <div class="row <?=$rowData['CLASS']?>" data-entity="items-row"<?=(in_array($rowData['VARIANT'], $arResizeRow))?" data-resize='1'":""?>>
                    <?
//                        if ($rowData['VARIANT']>0)
                            include('row_templates/row_'.$rowData['VARIANT'].'.php');
                    ?>
                </div>
                <?
        }

		$elementDraw->showElement("elements", $arParams['BXR_STANDART']['BXREADY_ELEMENT_DRAW'], array(), array(), true);
...
<script>
	...
	var <?=$obName?> = new JCCatalogSectionComponentDef({
