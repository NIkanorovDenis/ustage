...
if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
    \Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:search.title");
	...
	for($i = 0; $i < $arParams["NUM_CATEGORIES"]; $i++)
	{
		...
		$arResult["CATEGORIES"][$i] = array(
			"TITLE" => htmlspecialcharsbx($category_title),
                        "TITLE_PARAMS" => htmlspecialcharsbx($arParams["CATEGORY_".$i."_TITLE"]),
			"ITEMS" => array()
		);
		...
		))
		{
			while($ar = $obTitle->Fetch())
			{
				...
				{
                                    $ar["NAME"] = "#start#".$ar["NAME"]."#end#";
                                    $ar["NAME"] = str_replace("</b>", "</b><span>", $ar["NAME"]);
                                    $ar["NAME"] = str_replace("<b>", "</span><b>", $ar["NAME"]);
                                    
                                    $ar["NAME"] = str_replace("#start#</span>", "", $ar["NAME"], $c);                                    
                                    if($c==0)
                                      $ar["NAME"] = "<span>".$ar["NAME"];
                                    
                                    $ar["NAME"] = str_replace("<span>#end#", "", $ar["NAME"], $c);
                                    if($c==0)
                                      $ar["NAME"] .= "</span>";
                                    
                                    $ar["NAME"] = str_replace(array("#end#", "#start#"), "", $ar["NAME"]);
	...
	if($arParams["SHOW_OTHERS"] === "Y")
	{
		$arResult["CATEGORIES"]["others"] = array(
			"TITLE" => htmlspecialcharsbx($arParams["CATEGORY_OTHERS_TITLE"]),
                        "TITLE_PARAMS" => htmlspecialcharsbx($arParams["CATEGORY_OTHERS_TITLE"]),
			"ITEMS" => array(),
		);
