...
if ($sort == 'PROPERTY_MINIMUM_PRICE') {
    ...
    if ($regionData['region_detail']['PROPERTY_BXR_SORT_PRICE_ID_VALUE'] > 0) {
        $sort = 'CATALOG_PRICE_'.$regionData['region_detail']['PROPERTY_BXR_SORT_PRICE_ID_VALUE'];
    }
}
...
if (is_array($arCurSection['SEO_DATA']['DESCR'])) {
	$arSectionParams['SEO_SECTION'] = "Y";
	$arSectionParams['SHOW_ALL_WO_SECTION'] = 'Y';
	if ($arCurSection['IBLOCK_SECTION_ID'] > 0) {
		//$arSectionParams['SECTION_ID'] = $arCurSection['IBLOCK_SECTION_ID'];
		//$arSectionParams['~SECTION_ID'] = $arCurSection['IBLOCK_SECTION_ID'];
		//$arSectionParams['SECTION_CODE'] = '';
	}
}
