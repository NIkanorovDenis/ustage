...
if(!empty($arParams["REGION"])) {
    if(is_array($arResult['ITEMS'])) {
        foreach ($arResult['ITEMS'] as $k => $v){
            if(!empty($v["PROPERTIES"]["BXR_REGION"]["VALUE"])) {
                if(!in_array($arParams["REGION"], $v["PROPERTIES"]["BXR_REGION"]["VALUE"])){}
                else {
                    if(isset($arResult['ITEMS'][$k]))
                        unset($arResult['ITEMS'][$k]);
                }
            }
        }        
        Bxmarket::storesElemenQuantity($arResult["ITEMS"], $arParams);
    }
}
		...
		if (!empty($sku) && is_array($sku))
		{
			...
			if(isset($arResult['ITEMS']) && is_array($arResult['ITEMS'])) {
				foreach ($arResult['ITEMS'] as $tmpItem){
					if(count($tmpItem['OFFERS'])>0 && is_array($tmpItem['OFFERS'])){
						foreach ($tmpItem['OFFERS'] as $val) {
							if(count($val['PROPERTIES'])>0 && is_array($val['PROPERTIES'])){
								foreach ($val['PROPERTIES'] as $vCell => $vProp){
									if (is_array($arParams['OFFER_TREE_PROPS'][$offersCatalogId]) && in_array($vCell, $arParams['OFFER_TREE_PROPS'][$offersCatalogId])) {
										if ($vProp['PROPERTY_TYPE'] == 'S' && strlen($vProp['VALUE']) > 0) {
											if (!is_array($needValues[$vProp["ID"]]) || !in_array($vProp['VALUE'], $needValues[$vProp["ID"]])) {
												$needValues[$vProp["ID"]][] = $vProp['VALUE'];
											}
										}
										if ($vProp['PROPERTY_TYPE'] == 'L' && $vProp['VALUE_ENUM_ID'] > 0) {
											if (!is_array($needValues[$vProp["ID"]]) || !in_array($vProp['VALUE_ENUM_ID'], $needValues[$vProp["ID"]])) {
												$needValues[$vProp["ID"]][] = $vProp['VALUE_ENUM_ID'];
											}
										}
									}
								}
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			...
			$arItem['OFFERS_PROP'] = false;
                        ...
                        $tmpIds = array();
                        foreach ($arItem['OFFERS'] as $i => $offer) {
                            if (in_array($offer["ID"], $tmpIds)) 
                                unset ($arItem['OFFERS'][$i]);
                            else
                                $tmpIds[] = $offer["ID"];
                        }
                        unset($tmpIds);
