...
$elementarArea = \Alexkova\Bxready2\Elementars::getArea('catalog.element','element.result.modifier');
...
foreach($arTabs['TABS'] as $cell=>$val) {
	...
	if ($cell == 'COLLECTION') {
		if ($arResult["PROPERTIES"]["BXR_COLLECTION"]["VALUE_XML_ID"] == "Y") {
			$arTabs['DETAIL']['COLLECTION'] = array(
				'CAPTION'=>$arParams['BXR_DETAIL_TABS']['BXR_DETAIL_TAB_COLLECTION_CAPTION'],
				'LINK'=>$arParams['BXR_DETAIL_TABS']['BXR_DETAIL_TAB_COLLECTION_LINK'],
				'GLYPH'=>$arParams['BXR_DETAIL_TABS']['BXR_DETAIL_TAB_COLLECTION_GLYPH'],
			);
			if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_TEXT_LINK"]) > 0) {
				$arSmartLink['COLLECTION'] = array(
					"NAME" => $arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_COLLECTION_LINK"],
					"LINK" => "COLLECTION",
					"TYPE" => "tab",
					"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_COLLECTION_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_COLLECTION_GLYPH"]:"fa fa-cube",
				);
			}

			unset($arTabs['TABS']['STORES']);
			unset($arTabs['TABS']['SKU']);
		}
		else
		{
			unset($arTabs['TABS']['COLLECTION']);
		}
		continue;
	}
	...
	if ($cell == 'DETAIL') {
		...
		if ($arParams['BXR_DETAIL_TABS']['BXR_DETAIL_TAB_TEXT'] != 'tabs'){
			...
			if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_TEXT_LINK"]) > 0)
				$arSmartLink['DETAIL'] = array(
					"NAME" => $arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_TEXT_LINK"],
					"LINK" => "DETAIL",
					"TYPE" => "tab",
					"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_TEXT_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_TEXT_GLYPH"]:"fa fa-info",
				);
		}
...
if ($arParams["BXR_DETAIL_TABS"]['BXR_DETAIL_TAB_REVIEWS'] == "Y" && $arParams["USE_COMMENTS"] != "Y")
    unset($arTabs['TABS']['REVIEWS']);
elseif (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_REVIEWS_LINK"]) > 0)
    $arSmartLink['REVIEWS'] = array(
        "NAME" => $arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_REVIEWS_LINK"],
        "LINK" => "REVIEWS",
        "TYPE" => "tab",
        "GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_REVIEWS_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_REVIEWS_GLYPH"]:"fa fa-comment-o",
    );

if (isset($arParams["USE_STORE"]) && $arParams["USE_STORE"]=="Y" && strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_STORES_LINK"]) > 0 && $arResult["PROPERTIES"]["BXR_COLLECTION"]["VALUE_XML_ID"] !== "Y") {
    $arSmartLink['STORES'] = array(
        "NAME" => $arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_STORES_LINK"],
        "LINK" => "STORES",
        "TYPE" => "tab",
        "GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_STORES_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_STORES_GLYPH"]:"fa fa-archive",
    );
}
else
   unset($arTabs['TABS']['STORES']); 
...
if ($arParams["BXR_DETAIL_TABS"]['BXR_DETAIL_TAB_FILES'] == "Y"){
	...
	$filePrCode = "BXR_FILES";
	if(isset($arParams["OWN_CODES_SERVICE_PROPERTIES"]) && $arParams["OWN_CODES_SERVICE_PROPERTIES"] == "Y") {
		$filePrCode = (!empty($arParams["OWN_CODES_BXR_FILES"])) ? $arParams["OWN_CODES_BXR_FILES"] : $filePrCode;
	}

    if (CModule::IncludeModule('alexkova.market2')){
        $ufProperty = \Alexkova\Market2\Core::getUfSection($arParams["IBLOCK_ID"], $arResult["IBLOCK_SECTION_ID"], "UF_FILES");
        if(is_array($ufProperty) && !empty($ufProperty)) {            
            if(!empty($arResult['PROPERTIES'][$filePrCode]['VALUE']))
                $arResult["PROPERTIES"][$filePrCode]["VALUE"] = array_merge($arResult["PROPERTIES"][$filePrCode]["VALUE"], $ufProperty);
            else
                $arResult["PROPERTIES"][$filePrCode]["VALUE"] =  $ufProperty;
        }
    }
    
	if (is_array($arResult['PROPERTIES'][$filePrCode]['VALUE']) && count($arResult['PROPERTIES'][$filePrCode]['VALUE'])>0){
		foreach($arResult['PROPERTIES'][$filePrCode]['VALUE'] as $cell => $val){
			$arFileData = CFile::GetFileArray($val);
			$arFileData['DESCRIPTION'] = $arResult['PROPERTIES'][$filePrCode]['DESCRIPTION'][$cell];
		...
		if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_FILES_LINK"]) > 0){
			$arSmartLink['FILES'] = array(
				...
				"LINK" => "FILES",
				"TYPE" => "tab",
				"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_FILES_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_FILES_GLYPH"]:"fa fa-file-text-o",
			);
		}
	}else{
...
if ($arParams["BXR_DETAIL_TABS"]['BXR_DETAIL_TAB_SCHEMES'] == "Y"){
	...
	$schemesPrCode = "BXR_SCHEMES";
	if(isset($arParams["OWN_CODES_SERVICE_PROPERTIES"]) && $arParams["OWN_CODES_SERVICE_PROPERTIES"] == "Y") {
		$schemesPrCode = (!empty($arParams["OWN_CODES_BXR_SCHEMES"])) ? $arParams["OWN_CODES_BXR_SCHEMES"] : $schemesPrCode;
	}

    if (CModule::IncludeModule('alexkova.market2')){
        $ufProperty = \Alexkova\Market2\Core::getUfSection($arParams["IBLOCK_ID"], $arResult["IBLOCK_SECTION_ID"], "UF_SCHEMES");

        if(is_array($ufProperty) && !empty($ufProperty)) {            
            if(!empty($arResult['PROPERTIES'][$schemesPrCode]['VALUE']))
                $arResult["PROPERTIES"][$schemesPrCode]["VALUE"] = array_merge($arResult["PROPERTIES"][$schemesPrCode]["VALUE"], $ufProperty);
            else
                $arResult["PROPERTIES"][$schemesPrCode]["VALUE"] =  $ufProperty;
        }
    }

	if (is_array($arResult['PROPERTIES'][$schemesPrCode]['VALUE']) && count($arResult['PROPERTIES'][$schemesPrCode]['VALUE'])>0){
		foreach($arResult['PROPERTIES'][$schemesPrCode]['VALUE'] as $cell => $val){
		...
		if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SCHEMES_LINK"]) >0){
			$arSmartLink['SCHEMES'] = array(
				...
				"LINK" => "SCHEMES",
				"TYPE" => "tab",
				"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SHEMES_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SHEMES_GLYPH"]:"fa fa-folder-open-o",
			);
		}
	}else{
...
if ($arParams["BXR_DETAIL_TABS"]['BXR_DETAIL_TAB_VIDEO'] == "Y"){
	...
	$videoPrCode = "BXR_VIDEO";
	if(isset($arParams["OWN_CODES_SERVICE_PROPERTIES"]) && $arParams["OWN_CODES_SERVICE_PROPERTIES"] == "Y") {
		$videoPrCode = (!empty($arParams["OWN_CODES_BXR_VIDEO"])) ? $arParams["OWN_CODES_BXR_VIDEO"] : $videoPrCode;
	}

    if (CModule::IncludeModule('alexkova.market2')){
        $ufProperty = \Alexkova\Market2\Core::getUfSection($arParams["IBLOCK_ID"], $arResult["IBLOCK_SECTION_ID"], "~UF_VIDEO");
        ...
        if(is_array($ufProperty) && !empty($ufProperty)) {
            ...
            if(!empty($arResult['PROPERTIES'][$videoPrCode]['VALUE']))
                $arResult["PROPERTIES"][$videoPrCode]["VALUE"] = array_merge($arResult["PROPERTIES"][$videoPrCode]["VALUE"], $ufProperty);
            else
                $arResult["PROPERTIES"][$videoPrCode]["VALUE"] =  $ufProperty;
        }
    }

	if (is_array($arResult['PROPERTIES'][$videoPrCode]['VALUE']) && count($arResult['PROPERTIES'][$videoPrCode]['VALUE'])>0){
		foreach($arResult['PROPERTIES'][$videoPrCode]['VALUE'] as $cell => $val){
			$arResult["VIDEO"][] = $val;
		};
		if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_VIDEO_LINK"])>0){
			$arSmartLink['VIDEO'] = array(
				...
				"LINK" => "VIDEO",
				"TYPE" => "tab",
				"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_VIDEO_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_VIDEO_GLYPH"]:"fa fa-film",
			);
		}
	}else{
	...
	if (count($arResult['DISPLAY_PROPERTIES'])>0){
		if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_PROPERTIES_LINK"]) >0){
			$arSmartLink['PROPERTIES'] = array(
				...
				"LINK" => "PROPERTIES",
				"TYPE" => "tab",
				"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_PROPERTIES_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_PROPERTIES_GLYPH"]:"fa fa-list",
			);
		}
	}else{
	...
	if (count($arResult['OFFERS'])>0){
		if (strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SKU_LINK"]) > 0){
			$arSmartLink['SKU'] = array(
				...
				"LINK" => "SKU",
				"TYPE" => "tab",
				"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SKU_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_SKU_GLYPH"]:"fa fa-snowflake-o",
			);
		}
	}else{
		unset($arTabs['TABS']['SKU']);
...
if ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_INC"] == "Y" && strlen($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_INC_LINK"]) > 0){
	$arSmartLink['INC'] = array(
		"NAME" => $arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_INC_LINK"],
		"LINK" => "INC",
		"TYPE" => "tab",
		"GLYPH" => ($arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_INC_GLYPH"])?$arParams["BXR_DETAIL_TABS"]["BXR_DETAIL_TAB_INC_GLYPH"]:"fa fa-info-circle",
	);
}

foreach ($arTabs["TABS"] as $tabCode => $arTab) {
    if (is_array($arSmartLink[$tabCode]))
        $arResult['SMART_LINK'][$tabCode] = $arSmartLink[$tabCode];
}

$arResult['TABS'] = $arTabs;

$arResult["TABS"]["EPILOG"] = array(
    //"REVIEWS"  
);

$arResult["TABS"]["ECHO"] = array(
    "DETAIL" => ($arResult["DETAIL_TEXT"]) ? '<meta itemprop="description" content="'.substr(strip_tags($arResult["DETAIL_TEXT"]), 0, 200).'">' : '<meta itemprop="description" content="'.$arResult["NAME"].'">'
);
...
/*start complect*/
$arComplectItems = array();
$arComplect = CCatalogProductSet::getAllSetsByProduct($arResult["ID"], 1);
if (is_array($arComplect) && !empty($arComplect))
{
    $currentComplect = current($arComplect)["ITEMS"];
    foreach($currentComplect as $complect) {
        $arComplectItems[$complect["ITEM_ID"]] = $complect["SORT"];
    }
}
if (!empty($arComplectItems))
{
    ...
    $arFilter = array("ID" => array_keys($arComplectItems));
    $arSelect = Array("ID", "IBLOCK_ID", "NAME", "SORT", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "DETAIL_PICTURE");
    foreach($arResult["CAT_PRICES"] as $price)
    {
        if ($price['CAN_VIEW'] && $price['CAN_BUY']) 
            $arSelect[] = $price["SELECT"];
    }
    $db_res = CIBlockElement::GetList(Array("SORT" => "ASC"), $arFilter, false, false, $arSelect);
    while ($item = $db_res->GetNext()) {
        if (!is_array($arResult["COMPLECT_ITEMS"][$item["ID"]])) {
            if (is_array(CCatalogSKU::GetInfoByOfferIBlock($item["IBLOCK_ID"]))) {
                $db_props = CIBlockElement::GetProperty($item["IBLOCK_ID"], $item["ID"], array("sort" => "asc"), Array("CODE" => "CML2_LINK"));
                if($arProp = $db_props->Fetch())
                    $pid = $arProp["VALUE"];
                $res = CIBlockElement::GetByID($pid);
                if($arProduct = $res->GetNext())
                    $detailPageUrl = $arProduct["DETAIL_PAGE_URL"];
                $item["DETAIL_PAGE_URL"] = ($bxr_use_links_sku_sef == "Y") ? (($bxr_use_links_sku_sef_code == "Y") ? $detailPageUrl.$arParams["OFFER_MASK"]."/".$item["CODE"]."/" : $detailPageUrl.$arParams["OFFER_MASK"]."/".$item["ID"]."/") : $detailPageUrl."?".$arParams["OFFER_REQUEST_MASK"]."=".$item["ID"];
            }
            ...
            $item["SORT"] = $item["~SORT"] = $arComplectItems[$item["ID"]];
            $item["CNT"] = 1;
            if ($item["PREVIEW_PICTURE"])
                $item["PREVIEW_PICTURE"] = array("ID" => $item["PREVIEW_PICTURE"]);
            if ($item["DETAIL_PICTURE"])
                $item["DETAIL_PICTURE"] = array("ID" => $item["DETAIL_PICTURE"]);
            $item["PRICES"] = CIBlockPriceTools::GetItemPrices($item["IBLOCK_ID"], $arResult["CAT_PRICES"], $item, $arParams['PRICE_VAT_INCLUDE'], $arConvertParams);
            $arResult["COMPLECT_ITEMS"][$item["ID"]] = $item;
        } else 
            $arResult["COMPLECT_ITEMS"][$item["ID"]]["CNT"]++;
    }
    \Bitrix\Main\Type\Collection::sortByColumn($arResult["COMPLECT_ITEMS"], array('SORT' => SORT_ASC));
}
/*end complect*/
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
    ...
    $arDouble = array();
    foreach ($arResult['OFFERS'] as $arOfferGroup)
    {
        $arOfferGroup['ID'] = intval($arOfferGroup['ID']);
        if (isset($arDouble[$arOfferGroup['ID']]))
            continue;
        $arIDS[] = $arOfferGroup['ID'];
    }
    unset($arOfferGroup);
    if (!empty($arIDS) && CBXFeatures::IsFeatureEnabled('CatCompleteSet'))
    {
        $arOfferSet = array_fill_keys($arIDS, false);
        ...
        if ($arOfferSet[$arResult['ID']])
        {
            foreach ($arOfferSet as &$setOfferValue)
            {
                if ($setOfferValue === false)
                    $setOfferValue = true;
            }
            unset($setOfferValue);
            unset($arOfferSet[$arResult['ID']]);
        }
        if ($arResult['OFFER_GROUP'])
        {
            $arOfferSet = array_filter($arOfferSet);
            $arResult['OFFER_GROUP_VALUES'] = array_keys($arOfferSet);
        }
    }

    foreach ($arResult['OFFERS'] as $keyOfferGroup => $arOfferGroup)
    {
        if (isset($arOfferSet[$arOfferGroup['ID']]))
        {
            $arOfferGroup['OFFER_GROUP'] = true;
            $arResult['OFFERS'][$keyOfferGroup]['OFFER_GROUP'] = true;
        }
    }
    unset($keyOfferGroup);
    unset($arOfferGroup);
...
if (is_object($cp))
{
	...
	$arResult['UNCACHED_PROPS'] = array(
		'BXR_COLLECTION' => $arResult["PROPERTIES"]["BXR_COLLECTION"]["VALUE_XML_ID"]
	);
