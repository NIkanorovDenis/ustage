...
<div id="<?=$arItemIDs["SLIDER_WRAP_ID"]?>" class="bxr-element-slider">
    ...
    <?if (count($arResult["MORE_PHOTO"]) > 1 && (!isset($arParams["SHOW_SLIDER_SYNCING"]) || $arParams["SHOW_SLIDER_SYNCING"] == "Y" || $arParams["SHOW_SLIDER_SYNCING"] == "D")) :?>
        <div id="<?=$arItemIDs["SLIDER_NAV_CONT_ID"]?>" class="bxr-element-slider-nav nav-hor <?=(!isset($arParams["SHOW_SLIDER_SYNCING"]) || $arParams["SHOW_SLIDER_SYNCING"] == "D") ? "hidden-xs" : "";?> ">
            <?foreach ($arResult["MORE_PHOTO"] as $key => $val):
...
<?if (count($arResult["MORE_PHOTO"]) > 0):?>
    <script>
		...
		<?if(!isset($arParams["SHOW_SLIDER_SYNCING"]) || $arParams["SHOW_SLIDER_SYNCING"] == "Y" || $arParams["SHOW_SLIDER_SYNCING"] == "D"):?>
