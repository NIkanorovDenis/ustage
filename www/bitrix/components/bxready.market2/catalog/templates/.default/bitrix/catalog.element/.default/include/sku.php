...
<?if (!empty($arResult["OFFERS"])) {?>
    <div id="<?=$arItemIDs['SKU_WRAP']?>" class="bxr-market-sku-select-wrap">
        ...
        <?if ($arParams["OFFERS_VIEW"] == "CHOISE") {?>
            <div class="sku-choise-wrap">
                <?foreach ($arResult["SKU_PROPS_LIST"] as $code => $prop) {?>
                    <?if(strripos($prop["CODE"], "_BXR_CHOISE_L")):?>
                    <div class="sku-prop" data-prop-id="<?=$prop["ID"]?>" data-prop-code="<?=$prop["CODE"]?>" data-prop-name="<?=$prop["NAME"]?>" data-prop-type="<?=$prop["PROPERTY_TYPE"]?>">
                        <ul class="sku-prop-values-list sku-prop-values-list-style-select bxr-color-after">
                            <li class="init"><?=GetMessage('CHOOSE').' '.mb_strtolower($prop["NAME"], LANG_CHARSET);?></li>
                    ...
                    <?else:?>
                        <div class="sku-prop" data-prop-id="<?=$prop["ID"]?>" data-prop-code="<?=$prop["CODE"]?>" data-prop-name="<?=$prop["NAME"]?>" data-prop-type="<?=$prop["PROPERTY_TYPE"]?>">
                            ...
                            <ul class="sku-prop-values-list">
                                <?foreach ($prop["VALUES"] as $valId => $propVal) {
                                    $valueType = ($propVal["FILE"]) ? "prop-img-wrap" : "prop-text-wrap";?>
                                    <li class="sku-prop-value <?=$valueType?> <?=$addClass?> bxr-border-color-hover" data-prop-code="<?=$code?>" data-prop-type="<?=$prop["PROPERTY_TYPE"]?>" data-val-id="<?=$propVal["ID"]?>" data-val-code="<?if(!empty($propVal["XML_ID"])) echo $propVal["XML_ID"]; else echo $propVal["NAME"];?>" data-val-name="<?=$propVal["NAME"]?>">
                                        <?if ($propVal["FILE"]) {?>
                                            <span class="prop-checked"></span>
                                            <img src="<?=$propVal["FILE"]?>" alt="<?=$propVal["NAME"]?>" title="<?echo $prop["NAME"].": ".($propVal["DESCRIPTION"]?$propVal["DESCRIPTION"]:$propVal["NAME"])?>">
                                        <?} else {
                ...
                <ul class="bxr-sku-icons-items">
                    <?foreach($arResult["OFFERS"] as $offer):
                        ...
                        <li class="bxr-sku-icons-item" data-pid="<?=$offer['ID']?>">
                            <div class="bxr-offers-ico bxr-border-color-hover <?=$addClass?>">
                                <img src="<?=$src?>" alt="<?=$offer['NAME']?>" title="<?=$offer['NAME']?>">
                            </div>
