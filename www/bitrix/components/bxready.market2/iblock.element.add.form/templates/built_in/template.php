...
<div class='bxr-form-body-container'>
	...
	if (!empty($arResult["ERRORS"])):?>
		<div class="bxr-form-errors"><?
			foreach ($arResult["ERRORS"] as $k => $v) {
				echo "<p><i class='fa fa-exclamation-triangle'></i> " . $v . "</p>";// ShowError($v);
			}        
			?>
		</div>	
	<?endif;?>
	...
	<form id="<?=$arParams['IDENTITY']?>" class='bxr-form-body roxy2' name="iblock_add" action="<?=$arParams["POST_FORM_URI"]?>" method="post" enctype="multipart/form-data">
		
		<input type="hidden" name="FORM_ID" value="<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER?>"/>
		...
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			...
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				...
				<?if ($inputType != 'hidden') {?>			
					<?if (intval($propertyID) > 0):?><span class="bxr-pr-name"><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?></span><?else:?><span class="bxr-pr-name"><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?></span><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?> <span class="starrequired">*</span><?endif?>
				<?}?>
					...
					case "N":
						for ($i = 0; $i<$inputNum; $i++) {
							...
							<?if ($inputType != 'textarea') {?>
								<input class="form-control" type="<?=$inputType?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>" data-code="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]?>"/>
								<?if ($inputType != 'hidden') {?>
									
								<?}?>
							<?} else {?>
									<textarea  class="form-control" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" rows="5" data-code="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]?>" style="resize: none"><?=trim($value)?></textarea>
							<?}?>
		...
		<div class="bxr-button-group text-left">
			...
			<button onclick="BXReady.Market.formRefresh('<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER;?>');return false;" class="bxr-color bxr-color-button" id="submitForm_<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER;?>">
