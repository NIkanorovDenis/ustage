...
if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
    \Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:iblock.element.add.form");
...
if ($arParams['PERSONAL_DATA'] == "Y") {
    ...
    $arResult['PERSONAL_SETTINGS'] = array(
		...
		'CAPTION2' => $arParams['PERSONAL_DATA2_TEXT'],
        'ACCEPT_DOCUMENT' => $arParams['PERSONAL_DATA_CAPTION'],
        'URL' => $arParams['PERSONAL_DATA_URL'],
		'ACCEPT_DOCUMENT2' => $arParams['PERSONAL_DATA2_CAPTION'],
        'URL2' => $arParams['PERSONAL_DATA2_URL'],
    );
} else
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["save"]) || !empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
		{
			...
			if ($bError)
			{
				$arResult["ERRORS"][$propertyID] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ?
$arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_REQUIRED"));
            ...
            {
                if($arParams["IBLOCK_ID"] == 34)
                {
                    if($propertyID == 1991 && strlen($propertyValue) <= 1) $arResult["ERRORS"][] = "Поле 'Ваше имя' должно быть больше или равно 2 символов!";
                    if($propertyID == 1992 && strlen($propertyValue) <= 6) $arResult["ERRORS"][] = "Поле 'Телефон' должно быть больше или равно 7 символов!";
                }
            }
		}
        ...
        if ($arParams['PERSONAL_DATA'] == "Y") {
            if (htmlspecialchars($_REQUEST['accept_personal']) != 'yes')
                $arResult["ERRORS"]["PERSONAL"] = strlen($arParams['PERSONAL_DATA_ERROR'])>0 ? $arParams['PERSONAL_DATA_ERROR'] : GetMessage('IBLOCK_FORM_WRONG_PERSONAL_DATA');
			if (htmlspecialchars($_REQUEST['accept_personal2']) != 'yes'){
                $arResult["ERRORS"]["PERSONAL2"] = strlen($arParams['PERSONAL_DATA_ERROR'])>0 ? $arParams['PERSONAL_DATA_ERROR'].' ' : GetMessage('IBLOCK_FORM_WRONG_PERSONAL_DATA');
			}
        }
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					...
					usFormData::insert($arUpdateValues);
					/**/
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			while($IBPropertyRow = $IBProperty->Fetch()) {
				$arProperty[$IBPropertyRow['ID']]['CODE'] = $IBPropertyRow['CODE'];
				$arProperty[$IBPropertyRow['ID']]['TYPE'] = $IBPropertyRow['PROPERTY_TYPE'];
			}

			foreach ($arProperty as $propId => $propInfo)
			{
				if ($propInfo['TYPE'] == 'L' ){
					$propListValueDb = CIBlockProperty::GetPropertyEnum($propInfo['CODE'], Array(), Array("IBLOCK_ID"=>$arUpdateValues['IBLOCK_ID'], "ID"=>$arUpdateValues['PROPERTY_VALUES'][$propId]));
					if($propEnum = $propListValueDb->GetNext())
					{
						$arUpdateValues['LIST_PROPS'][$propId] = $propEnum['VALUE'];
					}
				}
			}

			if(!empty($arUpdateValues["PROPERTY_VALUES"]) && is_array($arUpdateValues["PROPERTY_VALUES"])) {
				foreach ($arUpdateValues["PROPERTY_VALUES"] as $propId=>$propVal)
				{
					if($propId == 'TARGET_URL')
						$eventFields["TARGET_URL"] = $propVal;
									else {
										if(isset($arProperty[$propId]['CODE']))
											$eventFields[$arProperty[$propId]['CODE']] = $propVal;
									}
			...
			if(!empty($arUpdateValues["LIST_PROPS"]) && is_array($arUpdateValues["LIST_PROPS"])) {
				foreach ($arUpdateValues["LIST_PROPS"] as $propId=>$propVal)
				{
					if($propId == 'TARGET_URL')
						$eventFields["TARGET_URL"] = $propVal;
									else {
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = htmlspecialcharsex($_REQUEST["strIMessage"]);
                ...
                $APPLICATION->SetAdditionalCSS($componentPath."/style.min.css");
