...
/* bxready check function */
if ($_CHECK) {
	foreach($_CHECK as $cell=>$val) {
		if (substr($cell, 0 , strlen($arParams['FILTER_NAME'])) == $arParams['FILTER_NAME']) {
			$arResult['SHORT_CHECK'][substr($cell, strlen($arParams['FILTER_NAME']), strlen($cell) - strlen($arParams['FILTER_NAME']))] = $val;
		}
	}
}
/*  */
...
elseif(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y" && $arParams["NO_AJAX"] != 'Y')
{
	$this->setFrameMode(false);
	ob_start();
