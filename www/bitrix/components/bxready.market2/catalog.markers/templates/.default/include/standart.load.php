...
if ($arParams['SINGLE_MODE'] == 'Y')
{
    ...
    switch ($arResult['PARENT']) {
        ...
        case 'SALELEADER':
            ...
            break;
        case 'TEST_DRIVE':
            $arrMarkers = array("!PROPERTY_TEST_DRIVE" => false);
            break;
        default: 
            break;
    }
    ...
    if (count($arResult["MARKERS_LIST"]) > 0): 
        ...
        <div id="bxr-markers-container">
            ...
            <div class="row hidden-xs" id="bxr-markers">
                <div class="col-xs-12 lent"><? //var_dump($arResult["MARKERS_LIST"]); ?>
                    <?foreach($arResult["MARKERS_LIST"] as $cell):
                        switch ($cell) {
                            case "ACTION":?>
                                <div class="bxr-markers-group"
                                     id="marc_tabSPECIALOFFER"
                                     data-slide="SPECIALOFFER"
                                     data-type="markers"
                                     data-state="<?if($first) echo "load"?>"><?=GetMessage('SPECIALOFFER_BUTTON')?></div>
                                <?
                                if (!$firstMarker) $firstMarker = "SPECIALOFFER";
                                break;
                            case "NEW":?>
                                <div class="bxr-markers-group"
                                     id="marc_tabNEWPRODUCT"
                                     data-slide="NEWPRODUCT"
                                     data-type="markers"
                                     data-state="<?if($first) echo "load"?>"><?=GetMessage('NEWPRODUCT_BUTTON')?></div>
                                <?
                                if (!$firstMarker) $firstMarker = "NEWPRODUCT";
                                break;
                            case "RECCOMEND":?>
                                <div class="bxr-markers-group"
                                     id="marc_tabRECOMMENDED"
                                     data-slide="RECOMMENDED"
                                     data-type="markers"
                                     data-state="<?if($first) echo "load"?>"><?=GetMessage('RECOMMENDED_BUTTON')?></div>
                                <?
                                if (!$firstMarker) $firstMarker = "RECOMMENDED";
                                break;
                            case "HIT":?>
                                <div class="bxr-markers-group"
                                     id="marc_tabSALELEADER"
                                     data-slide="SALELEADER"
                                     data-type="markers"
                                     data-state="<?if($first) echo "load"?>"><?=GetMessage('SALELEADER_BUTTON')?></div>
                                <?
                                if (!$firstMarker) $firstMarker = "SALELEADER";
                                break;
                            case "TEST":?>
                                    <div class="bxr-markers-group"
                                         id="marc_tabTEST_DRIVE"
                                         data-slide="TEST_DRIVE"
                                         data-type="markers"
                                         data-state="<?if($first) echo "load"?>"><?=GetMessage('TEST_DRIVE_BUTTON')?></div>
                                    <?
                                    if (!$firstMarker) $firstMarker = "TEST_DRIVE";
                                    break;
                            default ;
                        }
                        $first = false;
            ...
            foreach($arResult["MARKERS_LIST"] as $cell):
                switch ($cell) {
                    ...
                    case "TEST":?>
                        <div class="hidden-lg hidden-md hidden-sm">
                            <div id="bxr-mobile-name-TEST_DRIVE"
                                    class="bxr-marker-mobile-names bxr-color-button"
                                    data-slide="TEST_DRIVE"
                                    data-unicum="<?=$unicum?>">
                                <?=GetMessage('TEST_DRIVE_BUTTON')?>
                            </div>
                        </div>
                        <?
                        $m = 'TEST_DRIVE';
                ...
                <div id="mark-panel-<?=$m?>" class="bxr-carousel bxr-markers-list">
                    <?
                    if ($first) {
                        $arParams['UNICUM_POSTFIX'] = $arResult['PARENT'];
                        $arParams['FILTER_NAME'] = 'arrMarkers';
