...
if(\Bitrix\Main\Loader::includeModule("alexkova.bxready2"))
    \Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:basket.small");
...
if ($itemID>0){
	...
	switch($_REQUEST["action"]){
		case "add":
			...
			Alexkova\Market2\Basket::addToBasket($itemID, floatval($_REQUEST["quantity"]), $delayed, $_REQUEST["BASKET_PROPS"], $provider);
			$arResult["PRODUCT_INFO"] = Alexkova\Market2\Basket::getPorductInfoByProductId($itemID);
		...
		case "delete":
			...
			break;
 		case "delay":
			Alexkova\Market2\Basket::delayItem($itemID, $provider);
			break;
                case "favor":
			$favorList = Alexkova\Market2\Basket::favorItem($itemID);
			break;
		case "back":
			Alexkova\Market2\Basket::toCart($itemID, $provider);
			break;
		case "newqty":
			Alexkova\Market2\Basket::newQty($itemID, floatval($_REQUEST["quantity"]), $provider);
			break;
	}
}
else {
...
if($newBasketUserID>0){
	...
	if (isset($arParams['RELOAD_PRICE']) && $arParams['RELOAD_PRICE']=="Y") {
		$basketD7 = \Bitrix\Sale\Basket::loadItemsForFUser(\Bitrix\Sale\Fuser::getId(), \Bitrix\Main\Context::getCurrent()->getSite());
		$refreshStrategy = \Bitrix\Sale\Basket\RefreshFactory::create(\Bitrix\Sale\Basket\RefreshFactory::TYPE_FULL);
		$basketD7->refresh($refreshStrategy);
		$basketD7->save();
	}
...
$addPrice = round(intval($arResult["MIN_ORDER_PRICE"]) - $arResult["SUMM"], 2);
$arResult["ADD_PRICE_FORMATED"] = str_replace('#', $addPrice, $arResult["CURRENCY_FORMAT"]);

$optionMsg = COption::GetOptionString("alexkova.market2", "bxr_min_order_price_msg");
