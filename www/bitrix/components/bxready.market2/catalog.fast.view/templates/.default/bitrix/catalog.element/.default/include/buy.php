...
*/?>
    ...
    <? }
    if($hasOffers || $subscribe && !$useSubscribeBtn) {?>
        <button class="bxr-color-button bxr-detail-product-request<?=($hasOffers)?' bxr-hidden':''?>" value="<?=$arResult["ID"]?>"                         
            data-pid="<?=$arResult["ID"]?>" 
            data-oid=""
            data-toggle="modal" 
    ...
    if($hasOffers || !$subscribe) {   
        ...
        <?if ($useOneClick):?>
            <div class="bxr-basket-action">
                <button class="bxr-one-click-buy bxr-font-color-hover<?=($hasOffers)?' bxr-hidden':''?>" 
                        data-pid="<?=$arResult["ID"]?>" 
                        data-oid=""
                        data-toggle="modal" 
...
if($hasOffers)
{
    $arOfferTmp = $arResult["OFFERS"][intval($_GET["offer_id"])];
    if(intval($arOfferTmp["CATALOG_QUANTITY"]) == 0 && floatval($arOfferTmp["CATALOG_PRICE_1"]) == 0) $subscribe = true;
}
?>
<?if(!$subscribe):?>
<?$APPLICATION->IncludeComponent(
   ...
   "template1",
   Array(
      "AFTER_SEND_TITLE" => "Вы уже отправили запрос",
      "AVAILABLE_FIELDS" => array("NAME","EMAIL","PHONE","DESCRIPTION"),
      "BUTTON_SEND" => "Отправить!",
      "ID_PRODUCT" => $arResult['ID'],
      "REQUIRED_FIELDS" => array("EMAIL","PHONE"),
      "SUCCESS_TITLE" => "Ваша заявка сохранена. Мы уведомим вас при снижении цены!",
      "TITLE" => "Дорого? Нашли дешевле?"
   )
);?>
<?endif;?>
