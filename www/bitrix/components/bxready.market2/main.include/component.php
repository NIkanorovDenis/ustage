...
\Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:main.include");
...
$sRealFilePath = isset($_SERVER["REAL_FILE_PATH"]) ? $_SERVER["REAL_FILE_PATH"] : false;
...
if (!$bHasPath)
{
	...
	if ($arParams["AREA_FILE_SHOW"] == "page")
	{
				...
				if(!empty($arParams["REGION"])) {
                    $arParams["AREA_FILE_SUFFIX_REGION"] = $arParams["AREA_FILE_SUFFIX"] . "_" . $arParams["REGION"];
                
                    // if page in SEF mode check real path
                    if (!empty($sRealFilePath) && strlen($sRealFilePath) > 0)
                    {
                            $slash_pos = strrpos($sRealFilePath, "/");
                            $sFilePath = substr($sRealFilePath, 0, $slash_pos+1);
                            $sFileName = substr($sRealFilePath, $slash_pos+1);
                            $sFileName = substr($sFileName, 0, strlen($sFileName)-4)."_".$arParams["AREA_FILE_SUFFIX_REGION"].".php";
                    }
                    // otherwise use current
                    else
                    {
                            $sFilePath = $APPLICATION->GetCurDir();
                            $sFileName = substr($APPLICATION->GetCurPage(true), 0, strlen($APPLICATION->GetCurPage(true))-4)."_".$arParams["AREA_FILE_SUFFIX_REGION"].".php";
	...
	else
	{
		...
		$sFilePathTMP = $sFilePath;
                ...
                if(!empty($arParams["REGION"])) {
                    ...
                    $sFileName = "sect_".$arParams["AREA_FILE_SUFFIX_REGION"].".php";
                    $bFileFound = $io->FileExists($_SERVER['DOCUMENT_ROOT'].$sFilePath.$sFileName);
...
else
{
	...
	{        
                if(!empty($arParams["REGION"])) {
                    $arParams["PATH_REGION"] = str_replace(".php", "_".$arParams["REGION"].".php", $arParams["PATH"]);

                    if(strlen($arParams["PATH_REGION"]) > strlen($arParams["PATH"])) {
                        ...
                        $arParams['PATH_REGION'] = Rel2Abs($sFilePath, $arParams['PATH_REGION']);
        ...
        if(!empty($arParams["REGION"])) {
            ...
            if(strlen($arParams["PATH_REGION"]) > strlen($arParams["PATH"])) {
                ...
                $slash_pos = strrpos($arParams['PATH_REGION'], "/");
                $sFilePath = substr($arParams['PATH_REGION'], 0, $slash_pos+1);
                $sFileName = substr($arParams['PATH_REGION'], $slash_pos+1);
