...
<div class="bxr-element-slider-main <?= $fullsize ?>" style="height:<?= $arParams["BXR_SLIDER_HEIGHT"]; ?>px;">
    <? if (count($arResult["IMAGES"]) > 0): ?>
        <? foreach ($arResult["IMAGES"] as $key => $slide):
            ...
            <a href="<?= $slide["SRC"] ?>" data-rel="gallery"
               data-fancybox="bx-gallery" <? if ($key == 0) echo 'id="main-photo"' ?>>
                <img class="img-responsive bxr-zoom-img" src="<?= $slide["TMB"]["SRC"] ?>" title="<?= $imgTitle ?>"
                     alt="<?= $imgAlt ?>"
    ...
    <div class="bxr-element-slider-nav <?= $fullsize ?> hidden-xs">
        <? foreach ($arResult["IMAGES"] as $key => $slide):
            ...
            <div class="bxr-nav-element <? if ($key == 0) echo "bxr-slide-active" ?>">
                <div class="slide-wrap <? if ($key == 0) echo 'first-slide' ?>"
                     data-item="<?= $slide["ITEM_ID"] ?>" <? if ($key == 0) echo 'id="main-photo-small"' ?>>
                    <img class="img-responsive" src="<?= $slide["TMB"]["SRC"] ?>" title="<?= $imgTitle ?>"
                         alt="<?= $imgAlt ?>" itemprop="image">
                </div>
            </div>
        <? endforeach; ?>
    </div>
