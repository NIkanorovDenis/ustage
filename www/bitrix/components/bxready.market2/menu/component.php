...
\Alexkova\Bxready2\Component::prepareParams($arParams, "bxready.market2:menu.".$this->getTemplateName());

if(!isset($arParams['CACHE_SELECTED_ITEMS'])) {
	$arParams['CACHE_SELECTED_ITEMS'] = "N";
}
if(!isset($arParams["SHOW_LAST_LEVEL_BUTTONS"])) {
    $arParams['SHOW_LAST_LEVEL_BUTTONS'] = "N";
}
...
if(isset($arParams["SHOW_TREE"]) && $arParams["SHOW_TREE"] == "Y") {
	$arResult["TREE"] = $this->showTree($arResult);
}
...
if(isset($arParams["COMPONENT_TEMPLATE"])) {
    $replacement = array("left", "left_hover");

    if(in_array($this->getTemplateName(), $replacement) && in_array($arParams["COMPONENT_TEMPLATE"], $replacement))
        $this->setTemplateName($arParams["COMPONENT_TEMPLATE"]);
}
