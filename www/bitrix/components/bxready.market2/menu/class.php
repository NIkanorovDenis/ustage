...
class CMarketMenuComponent extends CBitrixComponent
{
	...
	public function setSelectedItems($bMultiSelect = false)
	{
        ...
        public function showTree($arResult) {
            $stack = array();
            $lvl = 1;
            ...
            foreach($arResult as $k => $v) {
                ...
                if(isset($v['PARAMS']['ico_light']))
                    $v['ico_light'] = $v['PARAMS']['ico_light'];
                
                if(isset($v['PARAMS']['ico_dark']))
                    $v['ico_dark'] = $v['PARAMS']['ico_dark'];
                
                if(isset($v['PARAMS']['ico_color']))
                    $v['ico_color'] = $v['PARAMS']['ico_color'];
                
                if(isset($v['PARAMS']['ico_font']))
                    $v['ico_font'] = $v['PARAMS']['ico_font'];
                
                if(isset($v['PARAMS']['hover']))
                    $v['hover'] = $v['PARAMS']['hover'];
                
                if(isset($v['PARAMS']['show_in']))
                    $v['show_in'] = $v['PARAMS']['show_in'];
                
                if(isset($v['PARAMS']['PICTURE']))
                    $v['IMG'] = $v['PARAMS']['PICTURE'];


                if($v["DEPTH_LEVEL"]==$lvl)
                    $stack[$lvl][] = $v;
                ...
                if($v["DEPTH_LEVEL"]<$lvl) {
                    for($i=count($stack); $i>1; $i--) {
                        ...
                        $stack[$i-1][count($stack[$i-1])-1]["CHILDREN"] = $stack[$i];    
                        unset($stack[$i]);					
                    }
