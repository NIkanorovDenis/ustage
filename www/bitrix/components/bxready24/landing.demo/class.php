...
class LandingSiteDemoComponent extends LandingBaseComponent
{
		...
		if (isset($demo[$code]))
		{
			...
			else
			{
				...
				)
				{
					...
					{
						$callbackRes = self::createCatalog($code);
						if ($callbackRes !== true)
						{
	...
	protected function getDemo($subDir)
	{
		...
		{
			$event = new \Bitrix\Main\Event('landing', 'onDemosGetRepositoryBxr24', array(
				'data' => $data
			));
			$event->send();
		...
		if (!isset($data[$subDir]))
		{
			...
			$pathLocal = '/bitrix/components/bxready24/landing.demo/data/' . $subDir;//@todo make better
