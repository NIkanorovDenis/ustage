...
$arComponentParameters = array(
	"GROUPS" => array(
		...
		"BXREADY_BOOTSTRAP_MODE" => array(
			"NAME" => GetMessage("BXREADY_BOOTSTRAP_MODE_SETTINGS"),
		),
...
/*$arParameters = \Alexkova\Bxready2\Lists::getListSettings(intval($arCurrentValues["BXREADY_LIST_BOOTSTRAP_GRID_STYLE"]), $arCurrentValues);

foreach($arParameters["LIST_GROUPS"] as $cell=>$val){
	$name = $val["NAME"];
	$val["NAME"] = GetMessage('BXREADY_NEWS_MAIN_PAGE').": ".$name;
	$arComponentParameters["GROUPS"][$cell."_MAIN"] = $val;
	$val["NAME"] = GetMessage('BXREADY_NEWS_SECTION_PAGE').": ".$name;
	$arComponentParameters["GROUPS"][$cell."_SECTION"] = $val;
}*/
